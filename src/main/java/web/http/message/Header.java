...
public class Header {
    ...
    public enum Type {
        ...
        // 일반 헤더
        CONNECTION("Connection"), DATE("Date"), MIME_VERSION("MIME-Version"), VIA("Via"),
        ...
        // 프락시 요청 헤더
        MAX_FORWARDS("Max-Forwards"), PROXY_AUTHORIZATION("Proxy-Authorization"), PROXY_CONNECTION("Proxy-Connection"),

        // 응답 헤더
        AGE("Age"), PUBLIC("Public"), RETRY_AFTER("Retry-After"), SERVER("Server"),

        // 협상 헤더
        ACCEPT_RANGES("Accept-Ranges"), VARY("Vary"),

        // 응답 보안 헤더
        PROXY_AUTHENTICATE("Proxy-Authenticate"), SET_COOKIE("Set-Cookie"),

        // 엔터티 헤더
        ALLOW("Allow"), LOCATION("Location"),

        // 콘텐츠 헤더
        CONTENT_BASE("Content-Base"), CONTENT_ENCODING("Content-Encoding"), CONTENT_LANGUAGE("Content-Language"),
        CONTENT_LENGTH("Content-Length"), CONTENT_LOCATION("Content-Location"), CONTENT_MD5("Content-MD5"),
        CONTENT_RANGE("Content-Range"), CONTENT_TYPE("Content-Type"),
